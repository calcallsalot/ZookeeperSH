...
function ensureSecretState(gs) {
  ...
  // in other words gives fas their cover role from roles that aren't taken already 
...
function maybeEmitDeckShuffle({ gs, lobbyId, emitGameSystem, shuffleCounts, isReshuffle }) {
  ...
  if (isReshuffle) {
    ...
    // Exile tokens/states are discarded after a reshuffle.
    clearAllExiles(gs);
...
function nextAlivePresidentSeat(players, currentSeat, exiledBySeat, exiledBySeat) {
  const alive = (players ?? [])
    .filter((p) => p.alive && exiledBySeat?.[p.seat] !== true && exiledBySeat?.[p.seat] !== true)
    .map((p) => p.seat)
    .sort((a, b) => a - b);
